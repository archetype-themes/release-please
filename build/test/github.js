...
(0, mocha_1.describe)('GitHub', () => {
    ...
    (0, mocha_1.describe)('commitsSince', () => {
        ...
        (0, mocha_1.it)('backfills commit files for pull requests rebased and merged', async () => {
            const graphql = JSON.parse((0, fs_1.readFileSync)((0, path_1.resolve)(fixturesPath, 'commits-since-rebase.json'), 'utf8'));
            req
                .post('/graphql')
                .reply(200, {
                data: graphql,
            })
                .get('/repos/fake/fake/commits/b29149f890e6f76ee31ed128585744d4c598924c')
                .reply(200, { files: [{ filename: 'abc' }] })
                .get('/repos/fake/fake/commits/27d7d7232e2e312d1380e906984f0823f5decf61')
                .reply(200, { files: [{ filename: 'def' }] });
            const targetBranch = 'main';
            const commitsSinceSha = await github.commitsSince(targetBranch, commit => {
            ...
            (0, chai_1.expect)(commitsSinceSha.length).to.eql(2);
            (0, chai_1.expect)(commitsSinceSha[0].files).to.eql(['abc']);
            (0, chai_1.expect)(commitsSinceSha[1].files).to.eql(['def']);
            snapshot(commitsSinceSha);
            req.done();
        });
    });
